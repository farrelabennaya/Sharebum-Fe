...
export default function DeleteSelectedModal({
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if ((e.key === "Enter" || e.key === "NumpadEnter") && !busy && count > 0)
        onConfirm?.();
    };
  ...
  }, [open, busy, count, onCancel, onConfirm]);

  const canDelete = !busy && count > 0;
  const title = count === 1 ? "Hapus 1 foto?" : `Hapus ${count} foto?`;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="del-selected-title"
    >
      {/* Overlay */}
      <div
          ...
          <div className="rounded-2xl bg-zinc-900/90 text-white border border-white/10">
            <div className="p-5 md:p-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3
                    id="del-selected-title"
                    className="font-semibold text-lg leading-tight"
                  >
                    {title}
                  </h3>
                  <p className="text-sm text-zinc-400 mt-1">
                    Foto yang dipilih akan dihapus dari album. Tindakan ini
